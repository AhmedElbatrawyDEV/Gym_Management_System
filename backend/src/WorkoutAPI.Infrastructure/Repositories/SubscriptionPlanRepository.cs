...
public class SubscriptionPlanRepository : BaseRepository<SubscriptionPlan>, ISubscriptionPlanRepository
{
    ...
    public async Task<IEnumerable<SubscriptionPlan>> GetByIdsAsync(List<Guid> planIds, CancellationToken cancellationToken)
    {
        if (planIds == null || !planIds.Any())
            return Enumerable.Empty<SubscriptionPlan>();

        return await _dbSet
            .Where(sp => planIds.Contains(sp.Id) && sp.IsActive)
            .ToListAsync(cancellationToken);
    }
